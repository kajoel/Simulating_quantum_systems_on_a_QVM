...
def count_opt_iterations(H, qvm, samples=None, disp_opt=False, compare_eig=False,
    ...
    if compare_eig is True:
        ...
        return len(vqe.vqe_run(ansatz, H_program, initial(H.shape[0]), samples=samples,
                               qc=qvm, disp=print, return_all=True)['iteration_params'])
...
def swep_parameters(H, qvm, num_para=20, start=-10, stop=10, samples=None, fig_nr=0, save=False):
    ...
    if H.shape[0] > 3:
        ...
        for i, p_1 in enumerate(parameters):
            mesh_1[i] += p_1
            mesh_2[i] = parameters
            exp_val[i] = [vqe.expectation(ansatz(np.array([p_1, p_2])), H, samples=samples, qc=qvm) for p_2 in
                          parameters]
...
if __name__ == '__main__':
    ...
    do = True
    if do:
        ans = count_opt_iterations(H, qc, compare_eig=True, samples=10000, disp_opt=True, xatol=1e-3, fatol=1e-2, maxiter=25)
