...
def count_opt_iterations(H, qvm, old_version = False,  samples=None, disp_run_info=False, 
                         return_dict = False, xatol=1e-2,fatol=1e-3,maxiter=10000):
    ...
    Keyword Arguments:
        old_version {bool} -- True if you are using qvm, Fase if qc
        samples {int} -- Number of samples (default: {None})
        disp_run_info {bool} -- Display output in each step of optimization (default: {False})
        ...
        return_dict {bool} -- Returns the entire dict with all data from every iteration
    Returns:
        iterations {int} -- Number of iterations before convergence
        if return_dict is True: Returns dict with all run-data
    """                     
    ...
    if old_version: result = vqe.vqe_run(ansatz, H_program, initial(H.shape[0]), samples=samples, 
                             qvm=qvm, disp=display_option, return_all=True)
    else: result = vqe.vqe_run(ansatz, H_program, initial(H.shape[0]), samples=samples, 
                             qc=qvm, disp=display_option, return_all=True)
...
def sweep_parameters(H, qvm, old_version=False, num_para=20, start = -10, stop = 10,samples = None, 
                    fig_nr = 0, save = False):
    ...
    if H.shape[0] > 3:
        ...
        if old_version:
            ...
            exp_val = [vqe.expectation(ansatz(np.array([para])), H, samples = samples, qc=qvm) for para in parameters]
        ...
        for i,p_1 in enumerate(parameters):
            ...
            mesh_2[i] = parameters
            if old_version:
                exp_val[i] = [vqe.expectation(ansatz( np.array([p_1,p_2]) ), H, samples = samples, qvm=qvm)
                              for p_2 in parameters]
            else:
                exp_val[i] = [vqe.expectation(ansatz( np.array([p_1,p_2]) ), H, samples = samples, qc=qvm)
                              for p_2 in parameters]  
...
def main2(samples=1000, sweep_params=100):
    ...
    j,V = 1,1
    H,_ = hamiltonian(j,V)
    sweep_parameters(H,qvm,old_version = True, samples=samples, num_para=sweep_params, start=-2,stop=2)
    
    result = count_opt_iterations(H,qvm, old_verion=True, samples=samples,fatol=1e-2,xatol=1e-3, return_dict=True)
    
    plt.scatter(result['iteration_params'],result['expectation_vals'], label='Steps in nelder mead')
    plt.legend()
